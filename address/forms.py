...
class AddressForm(forms.ModelForm):
        ...
        state = forms.ChoiceField(required=True, choices=STATE_CHOICES, widget=forms.Select(attrs={"placeholder":"State","label":"State", "class":"form-control form-control-sm"}), label="State", initial="LA")
        ...
        customer = forms.ModelChoiceField(required=False, queryset=Customer.objects.all(), widget=forms.Select(attrs={"placeholder":"Customer","label":"Customer", "class":"form-control form-control-sm"}), label="Customer")
        work_area_num = forms.ModelChoiceField(queryset=WorkArea.objects.all(), widget=forms.Select(attrs={"placeholder":"Work Area","label":"Work Area", "class":"form-control form-control-sm"}), label="Work Area")
        ...
        lot_num = forms.CharField(required=False, widget=forms.widgets.TextInput(attrs={"placeholder":"Lot","label":"Lot", "class":"form-control form-control-sm"}), label="Lot")
        provider = forms.ModelChoiceField(queryset=ElectricProvider.objects.all(), widget=forms.Select(attrs={"placeholder":"Electric Provider","label":"Electric Provider", "class":"form-control form-control-sm"}), label="Electric Provider")
        notes = forms.CharField (widget=forms.Textarea(attrs={"placeholder":"Notes","type":"textarea", "class":"form-control form-control-sm", "id":"inputEmail4"}), label="Notes")
